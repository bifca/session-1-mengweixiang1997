...
(APlayer.prototype.init = function() {
  function e(e) {
    ...
    (i = i > 0 ? i : 0),
      ...
      m.option.showlrc &&
        m.updateLrc.call(
          m,
          (parseFloat(m.playedBar.style.width) / 100) * m.audio.duration
        ),
      (m.element.getElementsByClassName(
        "aplayer-ptime"
  ...
  function i(e) {
    ...
    return (
      (a = document.body.scrollTop + document.documentElement.scrollTop), t - a
    );
  }
  if (
    ...
    for (
      var l = this.element.getElementsByClassName("aplayer-lrc-content")[0]
          .innerHTML,
        s = l.split(/\n/),
        n = /\[(\d{2}):(\d{2})\.(\d{2})]/,
        r = /](.*)$/,
      ...
      d &&
        c &&
        (this.lrcTime.push(
          60 * parseInt(d[1]) + parseInt(d[2]) + parseInt(d[3]) / 100
        ),
        this.lrcLine.push(c[1]));
    }
  }
  if (
    ((this.element.innerHTML =
      '<div class="aplayer-pic"><img src="' +
      this.music.pic +
  ...
  this.audio.addEventListener("durationchange", function() {
    1 !== m.audio.duration &&
      (m.element.getElementsByClassName(
        "aplayer-dtime"
      )[0].innerHTML = m.secondToTime(m.audio.duration));
  }),
    this.audio.addEventListener("loadedmetadata", function() {
      (m.element.getElementsByClassName("aplayer-author")[0].innerHTML =
        " - " + m.music.author),
        (m.loadedTime = setInterval(function() {
          var e =
            m.audio.buffered.end(m.audio.buffered.length - 1) /
            m.audio.duration;
    ...
    (this.pauseButton = this.element.getElementsByClassName(
      "aplayer-pause"
    )[0]),
  ...
  this.element
    ...
    .addEventListener("click", function(e) {
      ...
      (t = t > 0 ? t : 0),
        ...
        1 === t
          ? (h.className = "demo-icon aplayer-icon-volume-up")
          : (h.className = "demo-icon aplayer-icon-volume-down");
    }),
    h.addEventListener("click", function() {
      m.audio.muted
        ? ((m.audio.muted = !1),
          (h.className =
            1 === m.audio.volume
              ? "demo-icon aplayer-icon-volume-up"
              : "demo-icon aplayer-icon-volume-down"),
          m.updateBar.call(m, "volume", m.audio.volume, "height"))
        : ((m.audio.muted = !0),
          (h.className = "demo-icon aplayer-icon-volume-off"),
          m.updateBar.call(m, "volume", 0, "height"));
    }),
  ...
  (APlayer.prototype.play = function() {
    ...
    this.playedTime = setInterval(function() {
      e.updateBar.call(
        e,
        "played",
        e.audio.currentTime / e.audio.duration,
        "width"
      ),
        e.option.showlrc && e.updateLrc.call(e),
        (e.element.getElementsByClassName(
          "aplayer-ptime"
        )[0].innerHTML = e.secondToTime(e.audio.currentTime));
    }, 100);
  }),
